...
try:
    res_app_obj = CreatFlaskApp(service_name=SERVICE_NAME, db_driver=DB_DRIVER_NAME, db_user=DB_USER,
                                ...
                                db_name=DB_NAME, db_pool_size=POOL_SIZE, db_pool_max_overflow=MAX_OVERFLOW, retry_interval = RETRY_INTERVAL, base=Base)
        ...
        if res_app_obj.check_db_connectivity_and_retry():
            if res_app_obj.init_databases_for_service():
                if res_app_obj.create_tables_associated_to_db_model():
                    ...
                    registration_app_logger.info("Starting GRPC server for the Token-User service with the IP & PORT:: {0}:{1}".format("localhost", "8081"))
